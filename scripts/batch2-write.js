...
(async () => {
  ...
  const diffReport = JSON.parse(fs.readFileSync(path.join(rootDir, 'reports', 'batch2-proposed-diff.json'), 'utf8'));
  
  console.log('=== STARTING BATCH 2 FIRESTORE WRITE (PARTIAL UPDATE ONLY) ===');
  const results = [];

  for (const item of diffReport) {
    // Only process AUTO_WRITE or MATCHED_NO_NEW_FIELDS (to update lastCheckedAt/sourceIds)
    // REJECT_MATCH stores are updated with lastCheckedAt only to enforce cooldown without mutating data
  ...
  console.log('\n=== BATCH 2 FIRESTORE WRITE COMPLETE ===');
