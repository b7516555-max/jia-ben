...
(async () => {
  ...
  const diffReport = JSON.parse(fs.readFileSync(path.join(rootDir, 'reports', 'batch1-proposed-diff.json'), 'utf8'));
  
  console.log('=== STARTING BATCH 1 FIRESTORE WRITE (PARTIAL UPDATE ONLY) ===');
  const results = [];

  for (const item of diffReport) {
    if (item.action !== 'AUTO_WRITE' && item.action !== 'MATCHED_NO_NEW_FIELDS') {
      console.log(`Skipping [${item.name}] - Action: ${item.action}`);
      continue;
    }
    ...
    // 1. Fetch current document from Firestore to guarantee merge safety
    ...
    // 2. Prepare only missing fields to merge
    ...
    // Only set address if currently empty
    ...
    // Only set phone if currently empty
    if (!curData.phone && item.proposedUpdates.phone) {
      fieldsToUpdate.phone = item.proposedUpdates.phone;
      updateMaskFields.push('phone');
    }

    // Only set website if currently empty
    if (!curData.website && item.proposedUpdates.website) {
      fieldsToUpdate.website = item.proposedUpdates.website;
      updateMaskFields.push('website');
    }

    // Only set openingHours if currently empty
    if (!curData.openingHours && item.proposedUpdates.openingHours) {
      fieldsToUpdate.openingHours = item.proposedUpdates.openingHours;
      updateMaskFields.push('openingHours');
    }
    ...
    // Enrichment metadata update
    const enrichment = {
      ...(curData.enrichment || {}),
      lastCheckedAt: new Date().toISOString(),
      missingFields: newMissingFields,
      sourcesUsed: [...new Set([...(curData.enrichment?.sourcesUsed || []), 'foursquare'])],
      version: 1
    };
    ...
    // 3. Send patch request with updateMask
    ...
    if (!patchRes.ok) {
      ...
      console.log(`Successfully updated [${item.name}] (${item.jiaPlaceId}) with fields: ${Object.keys(fieldsToUpdate).join(', ')}`);
      results.push({ name: item.name, jiaPlaceId: item.jiaPlaceId, status: 'updated', updatedFields: Object.keys(fieldsToUpdate) });
    }
  }
